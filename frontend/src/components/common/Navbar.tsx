...
export default function Navbar() {
  ...
  return (
    <nav className="h-14 bg-surface-raised border-b border-brand-grey flex items-center px-6 gap-4 z-40 flex-shrink-0">
      <Link to="/" className="flex items-center gap-2 mr-4">
        ...
        <span className="font-semibold text-brand-dark text-lg tracking-tight">FrameSpace</span>
      </Link>

      <div className="flex items-center gap-1">
        <Link
          to="/dashboard"
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ease-spring duration-150 ${
        ...
        <Link
          to="/dashboard"
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ease-spring duration-150 flex items-center gap-1.5 ${
      ...
      <div className="ml-auto flex items-center gap-2">
        <span className="text-sm text-brand-grey-dark hidden sm:block">
          Hi, <span className="font-medium text-brand-dark">{user?.name?.split(' ')[0]}</span>
        </span>
